...
class ProductController():
    ...
    def create_product(self, product: ProductSchema) -> ProductSchema | dict: 
        db = SessionLocal()
    ...
    def get_product(self, product_id: int) -> ProductSchema | dict:
        db = SessionLocal()
    ...
    def update_product(self, product_id: int, model: ProductSchema) -> ProductSchema | dict:
        db = SessionLocal()
        product = db.query(Product).filter(Product.id == product_id).first()
        if not product:
            db.close()
    ...
    def delete_product(self, product_id: int) -> ProductSchema | dict:
        db = SessionLocal()
        product = db.query(Product).filter(Product.id == product_id).first()
        if not product:
            db.close()
